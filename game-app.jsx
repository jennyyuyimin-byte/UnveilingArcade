...
function GameApp({ themeId = 'cosmic', instanceId }) {
  ...
  // ── implicit identity + durable save (Tiers 0–2) ───────────────────────────
  // No login: on load we restore this device's server-side save (revealed
  ...
  // Spending a free pixel goes through the server (which owns the balance and
  // rolls the surprise); if there's no backend we fall back to local play.
  const buy = async (indexArg) => {
    const cur = peekState();
    const index = indexArg != null ? indexArg : window.randomCovered(cur);
    if (index == null || cur.revealed[index]) return;
    if (cur.freePixels > 0) {
      const res = await window.GameSync.reveal(id, { mode: 'free', indices: [index], covered: coveredList(cur) });
      if (res === null) { localFreeReveal(index); return; }       // no backend → local
      if (!res.ok) { toast(res.error === 'no budget' ? 'No free pixels left' : 'Couldn’t reveal'); return; }
      applyServerReveal(res, true);
      toast('Free pixel ✓');
      return;
    }
    ...
    const reward = window.rollReward(); // decorative sprinkle preview only — the real reward is rolled server-side on reveal
    set({ sheet: 'claim', pendingBuy: { count: 1, price, index, reward, packId: price < 1 ? 'single_half' : 'single' } });
  };

  // local fallback when /api isn't reachable (in-app preview, offline)
  const localFreeReveal = (index) => {
  ...
  // called by PaymentSheet / Stripe return after a successful payment. The
  // server authorizes the reveal against the credits Stripe actually recorded
  // (paidCredits), so a forged return can't unlock pixels. Falls back to local
  // reveal only when there's no backend (preview).
  const revealPaid = async (info) => {
    const cur0 = peekState();
    const covered = coveredList(cur0);
    const opts = info.kind === 'bundle'
      ? { mode: 'paid', n: info.credits || 0, covered }
      : { mode: 'paid', indices: info.index != null ? [info.index] : undefined, n: info.count || 1, covered };
    const res = await window.GameSync.reveal(id, opts);
    if (res === null) { localRevealPaid(info); return; }   // no backend → local
    if (!res.ok) { toast(res.error === 'no budget' ? 'Already revealed' : 'Couldn’t reveal — contact support'); return; }
    applyServerReveal(res, info.kind !== 'bundle' && (info.count || 1) === 1);
  };

  // local fallback (preview / offline) — original client-side reveal behavior
  const localRevealPaid = (info) => {
    if (info.kind === 'bundle') {
  ...
  // currently-covered tiles (candidates the server may reveal)
  const coveredList = (st) => { const a = []; for (let i = 0; i < TOTAL; i++) if (!st.revealed[i]) a.push(i); return a; };

  // apply an authoritative /api/reveal result: mark the tiles, adopt the
  // server's balances, and surface the surprise (single popup or batch recap).
  const applyServerReveal = (res, single) => {
    const b = res.balances || {};
    const picks = (res.rewards || []).map((r) => r.index);
    if (!picks.length) return;
    set((st) => {
      const rev = { ...st.revealed };
      picks.forEach((i) => (rev[i] = 'me'));
      return { ...st, revealed: rev,
        myPixels: b.myPixels != null ? b.myPixels : st.myPixels + picks.length,
        ...
        raffles: b.raffles != null ? b.raffles : st.raffles,
        raffleCodes: b.raffleCodes || st.raffleCodes,
        jackpots: b.jackpots != null ? b.jackpots : st.jackpots };
    });
    if (theme.celebrate === 'confetti') window.fireConfetti(hostRef.current, theme);
    if (single) {
      flashIdx(picks[0]);
      const rr = res.rewards[0];
      set({ reward: { shown: rr.shown, resolved: rr.resolved } });
    } else {
      picks.forEach((idx, k) => setTimeout(() => flashIdx(idx), k * 70));
      const tally = {};
      res.rewards.forEach((r) => { tally[r.resolved] = (tally[r.resolved] || 0) + 1; });
  ...
  // reveal N free pixels at once ("reveal all" button). Server-authorized; falls
  // back to local when there's no backend.
  const revealMany = async (n) => {
    const cur0 = peekState();
    if (cur0.freePixels <= 0) return;
    const res = await window.GameSync.reveal(id, { mode: 'free', n, covered: coveredList(cur0) });
    if (res === null) { localRevealMany(n); return; }
    if (!res.ok) { toast('Couldn’t reveal'); return; }
    applyServerReveal(res, false);
  };

  // local fallback (preview / offline)
  const localRevealMany = (n) => {
    const cur = peekState();
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (v && typeof v.paid === 'boolean') {
        ...
        const cur = peekState();
        await revealPaid(cur.pendingBuy && cur.pendingBuy.packId ? cur.pendingBuy : reconstruct(v));
      ...
      set((st) => ({ ...st, paidSessions: [...(st.paidSessions || []), session] }));
      const cur = peekState();
      if (cur.pendingBuy) await revealPaid(cur.pendingBuy);
      set({ sheet: null, pendingBuy: null });
      cleanUrl();
    })();
